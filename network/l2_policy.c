...
static inline bool frame_has_ipv6_addrs(const uint8_t *frame, size_t frame_len) {
  const size_t min_len = ipv6_destination_address_offset + ipv6_destination_address_size_bytes;
...
static inline const BmIpAddr *dst_ip_ptr(const uint8_t *frame, size_t frame_len) {
  if (!frame_has_ipv6_addrs(frame, frame_len)) {
    return NULL;
  }
...
static inline void set_ingress_nibble(uint8_t *ports_byte, uint8_t ingress_port_1_15) {
  // preserve egress (low nibble)
  *ports_byte = (uint8_t)((*ports_byte & 0x0F) | ((ingress_port_1_15 & 0x0F) << 4));
}
static inline void clear_ingress_nibble(uint8_t *ports_byte) { *ports_byte = (uint8_t)(*ports_byte & 0x0F); }
static inline void clear_egress_nibble(uint8_t *ports_byte)  { *ports_byte = (uint8_t)(*ports_byte & 0xF0); }

BmL2PolicyRxResult bm_l2_policy_rx_apply(uint8_t *frame,
                                        size_t frame_len,
                                        uint16_t ingress_port_mask,
                                        uint16_t all_ports_mask,
                                        L2LinkLocalRoutingCb routing_cb) {
  BmL2PolicyRxResult out = {
      .should_submit = true,
      .egress_mask = 0,
      .ingress_port_num = 0,
  };
  ...
  out.ingress_port_num = ingress_port_num;
  ...
  if (is_global_multicast(dst_ip)) {
    out.egress_mask = (uint16_t)(all_ports_mask & (uint16_t)~ingress_port_mask);
    // out.should_submit remains true
    return out;
  }
  ...
  if (routing_cb && !is_link_local_neighbor_multicast(dst_ip)) {
    ...
    out.should_submit = routing_cb(ingress_port_num, &egress, src_ip, dst_ip);
    out.egress_mask = egress;
