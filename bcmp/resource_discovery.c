...
static BmErr resource_discovery_reply_cb(BcmpProcessData data) {
  if (!data.payload) {
    return BmENODATA;
  }

  ResourceInfo *rep = (ResourceInfo *)data.payload;
  if (!rep) {
    return BmEINVAL;
  }
...
static BmErr bcmp_process_resource_request(BcmpProcessData data) {
  ...
  BcmpTxCtx ctx = {
      data.dst, BcmpResourceReplyMessage, (uint8_t *)rep, info_size,
      seq_num,  packet_null_cb(),         port_num,
  };
  ...
  ctx.seq_num = 0;
  ctx.egress_port = 1;
  ctx.reply_cb = packet_full_cb(resource_discovery_reply_cb);
...
BmErr bcmp_resource_discovery_send_request(uint64_t target_node_id,
  ...
  err = bcmp_tx(&multicast_ll_addr, BcmpResourceTableRequestMessage,
                (uint8_t *)&req, sizeof(req), 0, packet_null_cb());
  if (err == BmOK) {
...
BmErr bcmp_resource_send_request(const ResourceInfo *info) {
  ...
  BcmpTxCtx ctx = {
      ...
      .size = info_size,
      .seq_num = 0,
      .reply_cb = packet_full_cb(resource_discovery_reply_cb),
      .egress_port = 1,
  };
