...
TEST_F(BmLinuxInit, registers_packet_callbacks) {
  ...
  BcmpPacketCb cb = packet_init_fake.arg0_val;
  EXPECT_NE(cb.src_ip, nullptr);
  EXPECT_NE(cb.dst_ip, nullptr);
  EXPECT_NE(cb.data, nullptr);
  EXPECT_NE(cb.checksum, nullptr);
  EXPECT_NE(cb.increment, nullptr);
  EXPECT_NE(cb.decrement, nullptr);
  EXPECT_NE(cb.send, nullptr);
}
