...
// ============================================================================
// Real captured packet checksum regression tests
// ============================================================================

TEST_F(BmLinuxHelpers, ipv6_pseudo_checksum_real_packet1) {
  BmIpAddr src, dst;
  uint8_t src_bytes[16] = {
      0xFE, 0x80, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x62, 0x32, 0x67, 0x60, 0xDA, 0x4E, 0x23, 0x7A,
  };
  ...
  uint16_t expected_cksum = 0xF300;

  memcpy(src.addr, src_bytes, 16);
  memcpy(dst.addr, dst_bytes, 16);
  EXPECT_EQ(ipv6_pseudo_checksum(&src, &dst, ip_proto_bcmp, sizeof(data), data),
            expected_cksum);
}

TEST_F(BmLinuxHelpers, ipv6_pseudo_checksum_real_packet2) {
  BmIpAddr src, dst;
  uint8_t src_bytes[16] = {
      0xFE, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x55, 0xAA, 0x00, 0x11,
  };
  uint8_t dst_bytes[16] = {
      0xFF, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
  ...
  uint8_t data[] = {
      ...
      0x00, 0x00, 0x00, 0x00, 0x30, 0x7C, 0x71, 0x22, 0x00,
      0x00, 0x00, 0x00, 0x0A, 0x00, 0x00, 0x00,
  };
  uint16_t expected_cksum = 0x3F0C;
...
TEST_F(BmLinuxHelpers, ipv6_pseudo_checksum_real_packet3) {
  BmIpAddr src, dst;
  uint8_t src_bytes[16] = {
      0xFE, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x55, 0xAA, 0x00, 0x11,
  ...
  uint8_t data[] = {
      ...
      0x00, 0x00, 0x11, 0x00, 0xAA, 0x55, 0x00, 0x00, 0x00, 0x00, 0x01,
      0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x01, 0x00, 0x01, 0x05, 0x06, 0x30, 0x2E, 0x31, 0x2E,
      0x30, 0x62, 0x6D, 0x5F, 0x73, 0x62, 0x63,
  };
  uint16_t expected_cksum = 0x2DC8;
...
TEST_F(BmLinuxHelpers, ipv6_pseudo_checksum_captured_info_reply) {
  /* Captured from a live SBC→embedded exchange where the embedded node
     reported: "Packet checksum mismatch, read 0xB851, calculated 0x1AEF!"
     The correct checksum for these inputs is 0x1AEF. */
  BmIpAddr src, dst;
  uint8_t src_bytes[16] = {
      0xFE, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x5A, 0x02,
  };
  uint8_t dst_bytes[16] = {
      0xFF, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,
  };
  /* 91 bytes of BCMP (header + DeviceInfoReply), checksum field zeroed. */
  uint8_t data[] = {
      0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x02, 0x5A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x7A, 0x99, 0x28,
      0xF3, 0x00, 0x01, 0x00, 0x00, 0x22, 0x06, 0x6D, 0x75, 0x6C, 0x74,
      0x69, 0x6E, 0x6F, 0x64, 0x65, 0x40, 0x76, 0x30, 0x2E, 0x31, 0x2E,
      0x30, 0x2D, 0x35, 0x2D, 0x67, 0x66, 0x33, 0x32, 0x38, 0x39, 0x39,
      0x37, 0x61, 0x2D, 0x64, 0x69, 0x72, 0x74, 0x79, 0x62, 0x6D, 0x5F,
      0x73, 0x62, 0x63,
  };
  uint16_t expected_cksum = 0x1AEF;
